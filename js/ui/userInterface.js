...
import { generateCustomAdderTemplate } from '../handlers/customAdderHandler.js';
import { renderPhedAdderContainer } from '../handlers/phedHandler.js';
...
export function renderCollegeGeContent() {
    const college = document.getElementById("college-select").value;
    window.courseManager.selectedCollege = college;
...
export function createGrid(containerId, list) {
    ...
    container.innerHTML = list.map(item => {
        ...
        const state = window.courseManager.getGridCourseState(item.code);
...
export function handleContainerClick(event, code) {
...
export function toggleItemCheck(code) {
    const chk = document.getElementById(`chk-${code}`);
...
export function onGradeChange(code) {
    ...
    window.courseManager.setGridCourseState(code, checked, g1, g2);
...
export function updateCategoryCounts() {
    ...
    categories.forEach(cat => {
        ...
        if (grid && badge) {
            ...
            grid.querySelectorAll('.course-chk').forEach(chk => {
                if (window.courseManager.isCoursePassed(chk.value)) count++;
            });
            if (cat.gridId === "chlt-eltu-grid") {
                count += window.courseManager.langCourses.filter(c => window.courseManager.isCustomCoursePassed(c)).length;
            }
    ...
    ['faculty-req-grid', 'faculty-elec-grid'].forEach(gridId => {
        ...
        if (g) {
            g.querySelectorAll('.course-chk').forEach(chk => {
                if (window.courseManager.isCoursePassed(chk.value)) facultyCount++;
            });
        }
    });
    ...
    if (geBadge) {
        const ugePassedCount = window.courseManager.ugeCourses.filter(c => window.courseManager.isCustomCoursePassed(c)).length;
    ...
    if (phedBadge) {
        const phedPassedCount = window.courseManager.phedCourses.filter(c => window.courseManager.isCustomCoursePassed(c)).length;
        phedBadge.textContent = `${phedPassedCount} passed`;
        phedBadge.classList.toggle('active', phedPassedCount > 0);
    }
}

export function onStreamChange() {
    window.courseManager.selectedStream = document.getElementById("stream-select").value;
    if (document.getElementById("results-section").style.display === "block") {
        evaluateRequirements(false);
    }
}

export function onCollegeChange() {
    renderCollegeGeContent();
    if (document.getElementById("results-section").style.display === "block") {
        evaluateRequirements(false);
    }
...
export function toggleAllCategories(openState) {
